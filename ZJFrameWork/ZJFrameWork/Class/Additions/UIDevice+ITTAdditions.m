...
/*  Modified by Sword on 4/11/2013
    add method:
...
- (BOOL)is4InchScreen
{
    return (fabs((double)[ [ UIScreen mainScreen ] bounds ].size.height - 568.0) < DBL_EPSILON);
}
...
- (NSString*)deviceIdentifier
{
    ...
    NSString *identifier = infDic[@"CFBundleIdentifier"];
    if (!identifier||![identifier length]) {
        identifier = DEVICE_IDENTIFIER;
    }
    UIPasteboard *pasteboard = [UIPasteboard pasteboardWithName:identifier create:NO];
    if (!pasteboard) {
        ...
        pasteboard = [UIPasteboard pasteboardWithName:identifier create:YES];
        [pasteboard setPersistent:YES];
        [pasteboard setString:(NSString*)deviceIdentifier];
