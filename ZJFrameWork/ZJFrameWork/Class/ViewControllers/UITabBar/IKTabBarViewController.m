...
@interface UIViewController (IKTabBarControllerItemInternal)

- (void)ik_setTabBarController:(IKTabBarViewController *)tabBarController;
...
@property (nonatomic, readwrite) IKTabBar *tabBar;
...
- (void)setViewControllers:(NSArray *)viewControllers {
    if (viewControllers && [viewControllers isKindOfClass:[NSArray class]]) {
        ...
        for (UIViewController *viewController in viewControllers) {
            IKTabBarItem *tabBarItem = [[IKTabBarItem alloc] init];
            [tabBarItem setTitle:viewController.title];
            [tabBarItems addObject:tabBarItem];
            [viewController ik_setTabBarController:self];
        }
        
        [[self tabBar] setItems:tabBarItems];
    } else {
        for (UIViewController *viewController in _viewControllers) {
            [viewController ik_setTabBarController:nil];
        }
...
- (void)setTabBarHidden:(BOOL)hidden animated:(BOOL)animated {
    ...
    __weak IKTabBarViewController *weakSelf = self;
...
#pragma mark - UIViewController+IKTabBarControllerItem

@implementation UIViewController (IKTabBarControllerItemInternal)

- (void)ik_setTabBarController:(IKTabBarViewController *)tabBarController {
    objc_setAssociatedObject(self, @selector(ik_tabBarController), tabBarController, OBJC_ASSOCIATION_ASSIGN);
}

@end

@implementation UIViewController (IKTabBarControllerItem)

- (IKTabBarViewController *)ik_tabBarController {
    IKTabBarViewController *tabBarController = objc_getAssociatedObject(self, @selector(ik_tabBarController));
    
    if (!tabBarController && self.parentViewController) {
        tabBarController = [self.parentViewController ik_tabBarController];
    }
    
    return tabBarController;
}

- (IKTabBarItem *)ik_tabBarItem {
    IKTabBarViewController *tabBarController = [self ik_tabBarController];
    NSInteger index = [tabBarController indexForViewController:self];
    return [[[tabBarController tabBar] items] objectAtIndex:index];
}

- (void)ik_setTabBarItem:(IKTabBarItem *)tabBarItem {
    IKTabBarViewController *tabBarController = [self ik_tabBarController];
    ...
    IKTabBar *tabBar = [tabBarController tabBar];
    NSInteger index = [tabBarController indexForViewController:self];
